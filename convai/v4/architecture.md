...
flowchart TD
    subgraph Client["Client Layer"]
        Web["Web Frontend<br/>React / Vite :3000<br/>LiveKit JS SDK"]
    end
    ...
    subgraph Services["Service Layer"]
        ...
        LiveKit["LiveKit<br/>WebRTC SFU :7880<br/>Media :7881"]
    end
    ...
    %% Service connections
    RT_Text -->|HTTP| API
    RT_Text -.->|WebSocket| LiveKit
    RT_Text -->|HTTP| LLM
    RT_Text -->|gRPC| Guards

    %% Worker connections
    RT_Voice <-.->|WebSocket| LiveKit
    RT_Voice -->|HTTP| API
    RT_Voice -->|HTTP| LLM
    RT_Voice -->|gRPC| STT
    RT_Voice -->|gRPC| TTS
    RT_Voice -->|gRPC| Guards
...
flowchart TD
    ...
    LK["LiveKit SFU<br/>WebRTC audio transport"]
    ...
    subgraph Worker["Realtime-Voice Worker (Pipecat)"]
        ...
        TTS_Step["Text-to-Speech<br/>chatterbox_tts (Triton gRPC)<br/>streaming audio"]
    end
