...
flowchart TD
    ...
    subgraph Services["Application Services"]
        RT_Text["Realtime-Text<br/>WebSocket handler<br/>LLM ConversationProcessor<br/>Voice escalation"]
    ...
    subgraph Inference["Inference Workers (cascade mode)"]
        ...
        TTS["TTS (Triton)<br/>chatterbox_tts · gRPC :8001<br/>LoRA fine-tune + ACI"]
    end
    ...
    RT_Text -->|"HTTP tools + convos"| API
    RT_Voice -->|"HTTP tools + convos"| API
...
flowchart TD
    ...
    subgraph Worker["Realtime-Voice Worker"]
        ...
        Relay["TranscriptionRelay + TTSTimingRelay"]
    end
    ...
    Guard_svc["Guardrails (Triton)<br/>prompt_inj + multi_guard + pii<br/>gRPC :8001"]
