...
def cmd_layers(n_layers, *items_per_layer):
    ...
    base_item_h = 60
    computed_layer_h = 30 + 26 + base_item_h + 20  # = 136 per layer
    ...
    y = margin_y
    start_size = 30  # standard band startSize (single-line headers)
...
def cmd_header_size(*label_parts):
    ...
    Formula: max(30, 14 + n_lines × 16)
      1 line  → 30  (comfortable for fontSize=12 bold)
      2 lines → 46  (12px bold + 10px normal subtitle)
      3 lines → 62
    ...
    n_breaks = label.lower().count("<br/>") + label.lower().count("<br>")
    n_lines = n_breaks + 1
    start_size = max(30, 14 + n_lines * 16)

    print(f"n_lines={n_lines}")
    print(f"startSize={start_size}")
    print(f"# Formula: max(30, 14 + {n_lines} × 16) = {start_size}")
