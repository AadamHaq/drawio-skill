...
**Before starting, read these files** (in the same directory as this SKILL.md):
- `explore.md` — what to look for in the repo, topology classification rules
- `edge-rules.md` — edge styling, labels, max density per page
- `validation-checklist.md` — pre-write checklist (run EVERY item before saving)
- For PIPELINE repos: `pipeline/render.md`
- For MICROSERVICE repos: `microservice/render.md`
...
Use `layout.py` (in this directory) for all coordinate math:
```bash
python3 layout.py swimlanes <n>
python3 layout.py inputs <n>
python3 layout.py steps <sw_w> <startSize> <lines...>
python3 layout.py service-map <n_services> <layer_hints...>
python3 layout.py service-container <n_components>
python3 layout.py multipage <page_type>
```

### Step 4: Plan Edges
For cross-row/cross-service edges, write a JSON file with all node positions and edges, then run:
```bash
python3 edge_planner.py /tmp/edge_plan.json
```
...
```bash
python3 validate.py architecture.drawio
```
