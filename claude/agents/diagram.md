---
name: diagram
description: Analyse a repository and produce a draw.io architecture diagram. Use /diagram to generate, then /diagram-validate to check the output.
skills:
  - diagram
  - diagram-validate
---
...
1. Run `/diagram` — explores the repo, writes `diagram-spec.yaml`, runs `generate.py`
   to produce `Claude.drawio.xml`.
2. Run `/diagram-validate` — checks the XML for overlapping edges or edges routed
   through boxes, and reports any violations.
